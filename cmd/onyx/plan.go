...
func (cmd *PlanAddCmd) Run(cfg *config.Config, planCmd *PlanCmd) error {
    ...
    note, err := vault.GetDailyNote(planCmd.Date)
    if err != nil {
    ...
    // Time token mandatory; if missing interactive prompt.
    if cmd.TimeToken == "" {
    ...
    // Parse time token into start / optional end
    ...
    planner := obsidian.NewPlanner(note)
...
func splitTimeToken(token string) (string, string, error) {
    ...
    // validate chronological order (simple string compare sufficient for HH:MM format)
    if parts[1] <= parts[0] {
...
func (cmd *PlanListCmd) Run(cfg *config.Config, planCmd *PlanCmd) error {
    ...
    plannerMarkdown, err := planner.List()
    if err != nil {
        log.Error("Failed to list planner entries", "err", err)

        return fmt.Errorf("list planner: %w", err)
    }

    if len(plannerMarkdown) == 0 {
        plannerMarkdown = "## Day Planner\n\n(empty)"
    }

    if err := markdown.Render(os.Stdout, plannerMarkdown); err != nil {
